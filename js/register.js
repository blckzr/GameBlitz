...
(function () {
      ...
      .then(function (data) {
        if (!data.ok) {
          ...
          if (data.field === "email") {
            ...
            if (data.errors.name)
              setError("nameError", data.errors.name, nameField);
            if (data.errors.email)
              setError("emailError", data.errors.email, emailField);
            if (data.errors.password)
              setError("passwordError", data.errors.password, passwordField);
          } else {
  ...
  function showSuccess(name) {
    ...
    card.innerHTML =
      ...
      "<p>Welcome to GameBlitz, <strong>" +
      name.split(" ")[0] +
      "</strong>. Redirecting&hellip;</p>" +
